...
def cky(scores, mask):
    ...
    #print("shape of s pl",s.shape, p_s.shape, p_l.shape)
...
class CRFConstituency(nn.Module):
    ...
    def forward(self, scores, mask, target=None, require_marginals=False):
        ...
        #print("Shape of s logz",logZ.shape)
    ...
    def logsumexp(self, vec_mat):
        vec_mat = vec_mat.permute(1,0)
    ...
    def inside(self, scores, mask):
        ...
        # print("[mask inside algorithm", mask.shape)
        # print("[score inside algorithm", scores.shape)
        

        # working in the log space, initial s with log(0) == -inf
        s = torch.full_like(scores[:, :, 0], float('-inf'))
        # print("[s final inside algorithm", s.shape)
        for d in range(2, seq_len + 1): # d = 2, 3, ..., seq_len
            # define the offset variable for convenience
            offset = d - 1
            ...
            ### The commented block is the non-vectorized version of the same code.
            ## I implemented it first to understand the inside algorithm
            ## The vectorized version has been implemeneted below
            # if d == 2:
            #     for i in range(0,n):
            #         j = i + offset
            #         #print("inside",i,j,scores.shape, s.shape)
            #         s[i,j,:] = self.logsumexp(scores[i, j,:,:])  
            #         # print("inside",i,j,d, scores[i, j,:,:])
            #         # print("inside s",i,j,d, s[i, j,:])  
            # else:
            #     for i in range(0,n):
            #         j = i + offset
            #         log_sum_vals = self.logsumexp(scores[i, j,:,:])
            #         # print("inside",i,j,d, scores[i, j,:,:])
            #         # print("inside s",i,j,d, log_sum_vals)
            #         for k in range(i+1, j):
            #             add_vals = (log_sum_vals + s[i,k,:] + s[k, j, :]).view(1,-1)
            #             #print("Addvals",i,j,k, d, add_vals)
            #             final_vals = torch.cat((s[i,j,:].view(1,-1),add_vals), dim = 0)
            #             #print("final_ vals",i,j,k, d, final_vals)
            #             s[i,j,:] = self.logsumexp(final_vals)
            #             #print("inside s",i,j,k, d, s[i, j,:])


            #### This is the vectorized version of the code using logsumexp and stripe functions
            s_label = torch.logsumexp(scores.diagonal(offset),0)
            if d == 2:
                s.diagonal(offset).copy_(s_label)
            else:
                s_span = stripe(s, n, offset-1, (0, 1)) + stripe(s, n, offset-1, (1, offset), 0)
            # [batch_size, n, offset]
                s_span = s_span.permute(2, 0, 1)
                # [batch_size, n]
                s_span = torch.logsumexp(s_span,2)
                s.diagonal(offset).copy_(s_span + s_label)
        return s
...
class Model(nn.Module):
    ...
    def loss(self, scores, charts, mask, require_marginals=True):
        ...
        #print("scores shape", scores, "Charts shape:", charts.shape, "mask shape", mask.shape)
...
def train(model, traindata, devdata, optimizer):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for i, (words, *feats, trees, charts) in enumerate(traindata.loader):
            ...
            #print("Word mask shape", word_mask.shape)
            mask = word_mask if len(words.shape) < 3 else word_mask.any(-1)
            #print("Word mask shape2", word_mask.shape)
            mask = (mask.unsqueeze(1) & mask.unsqueeze(2)).triu_(1)
            #print("Word mask shape3", word_mask.shape)

            s_feat = model(words, feats)
            #print("sfeat", s_feat)
            loss, _ = model.loss(s_feat, charts, mask, require_marginals=False)
            #print("sfeat after", s_feat)
            loss.backward()
        ...
        up_dict[epoch] = dev_metric.up
        lp_dict[epoch] = dev_metric.lp
        ur_dict[epoch] = dev_metric.ur
        lr_dict[epoch] = dev_metric.lr
        uf_dict[epoch] = dev_metric.uf
        lf_dict[epoch] = dev_metric.lf
        lcm_dict[epoch] = dev_metric.lcm
        ucm_dict[epoch] = dev_metric.ucm
        print(f"{'dev:':5} loss: {loss:.4f} - {dev_metric}")

        t = datetime.now() - start
        if dev_metric > best_metric:
            best_e, best_metric = epoch, dev_metric
            torch.save(model.state_dict(), 'models/best_model_small_ass_3.pt')
        elapsed += t
    ...
    with open('metrics_plot.pkl','wb') as plotter:
        pickle.dump([train_loss, valid_loss, up_dict, lp_dict, ur_dict, lr_dict, uf_dict, lf_dict, lcm_dict, ucm_dict], plotter)
...
if __name__ == "__main__":
    ...
    p.add_argument('--epochs', default=20, type=int, help='the number of training epochs')
    ...
    print(model)
    print(args.n_tags, args.n_labels)
    transform.train()
    #print("..........", transform)
    traindata = Dataset(transform, args.train)
    ...
    ## This code is for evaluating the best performing model
    model.load_state_dict(torch.load('models/best_model_ass_3.pt', map_location=torch.device('cpu')))
    loss, test_metric = evaluate(model, testdata.loader)
    print(f"{'test:':5} loss: {loss:.4f} - {test_metric}")



## It prints the tree. Code is taken from the piazza post
    #create loader with batch_size as 1
    ...
    for i, tree in enumerate(ground_truth):
        ...
        if tree.sen_len() >=10 and tree.sen_len()<=15:
            #save trees as png files
            draw_tree(tree, res_path =f"./10-15/ground_truth_3_6_{i}.png")
            draw_tree(from_string(predictions[i]), res_path=f"./10-15/pred_3_6_{i}.png")
            print_tree_count-=1
